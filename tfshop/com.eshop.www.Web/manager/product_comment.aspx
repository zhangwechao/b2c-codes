<%@ Page Title="网站评论" Language="C#" MasterPageFile="~/manager/admin.master" AutoEventWireup="true" CodeFile="product_comment.aspx.cs" Inherits="product_comment" %>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#btnAdvSearch").bind("click", function() {
            ...
            window.location.href = "product_comment.aspx?" + query;
        });
        $("#btnSearch").bind("click", function() {
            var search = $.trim($("#txtSearch").val());
            window.location.href = "product_comment.aspx?content=" + search;
        });
...
$("#product_comment_page").css({ "background-image": "url(image/A-dmin-0122.gif)" });
</script>
 <div id="content">   
            ...
            <asp:Repeater ID="rData" runat="server" onitemcommand="rData_ItemCommand">
                ...
                <ItemTemplate>
                    <tr>
                        ...
                        <td title='<%#Eval("content") %>' style="cursor:pointer"><%#com.eshop.www.Tools.StringHelper.CutString(Eval("content").ToString(),50) %></td>
                        <td><%#Eval("IP") %></td>
                        <td><%#Eval("user_name")%></td>
                        <td><%#Eval("score") %>分</td>
                        <td><%#bool.Parse(Eval("is_show").ToString())?"是":"否"%></td>
                        <td><%#GetProductName(int.Parse(Eval("product_id").ToString())) %></td>
                        <td><%#DateTime.Parse(Eval("create_date").ToString()).ToString("yyyy-MM-dd") %></td>
                        <td>
                            <asp:Button ID="btnDelete" runat="server" Text="删除" CommandName="delete" CommandArgument='<%#Eval("Id") %>' OnClientClick="return ask()" />&nbsp;
                            <asp:Button ID="btnUpdateShow" runat="server" Text='<%#bool.Parse(Eval("is_show").ToString())?"隐藏":"显示" %>' CommandName="updateShow" CommandArgument='<%#Eval("Id") %>' />
                        </td>
                    </tr>
                </ItemTemplate>
            </asp:Repeater>
