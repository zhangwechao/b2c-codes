...
public class help : IHttpHandler
{
    ...
    private void Import()
    {
        ...
        if (file != null)
        {
            ...
            string sql = "select [标题],[摘要],[内容],[关键字],[目录] from [sheet1$]";
            ...
            foreach (DataRow row in excelTable.Rows)
            {
                ...
                if (title.Length != 0 && summary.Length != 0 && content.Length != 0 && keywords.Length != 0 && categoryName.Length!=0)
                {
                    orderBy = newsContentBusiness.GetMaxOrder();
                    categoryId = newsCategoryBusiness.GetEntityByCategoryName(categoryName).Id;
    ...
    {
        NewsCategoryBusiness newsCategoryBusiness = new NewsCategoryBusiness();
        string path = newsCategoryBusiness.GetEntity(categoryId).Path;
        string prefix = path.Substring(0, path.IndexOf(categoryId.ToString()) + categoryId.ToString().Length);
        DataRecordTable table = newsCategoryBusiness.GetList("Id,category_name", "order_by", false, 1, 100, "path like '" + prefix + "%'");
